...
-- Customers 테이블에서 Country가 Germerny 이고 City가 Berlin 인 행 조회
SELECT * FROM Customers WHERE Country = 'Germany' AND City = 'Berlin';

-- Customers 테이블에서 Country가 Germany 이고 City가 Berlin 인 행의 CustomerName 조회
...
-- Customers 테이블에서 Country가 Germany 이거나 Country가 Spain인 행 조회
SELECT * FROM Customers WHERE Country = 'Germany' OR Country = 'Spain';

-- NOT : 조건에 해당하지 않은 것
SELECT * FROM Customers WHERE NOT Country = 'UK';

-- Customers 테이블에서 Country가 Germany 가 아닌 해애 조회(NOT키워드 사용)
